...
data <- read.table("UCI HAR Dataset/features.txt", colClasses = "character")[,2]

train_x <- read.table("UCI HAR Dataset/train/X_train.txt", col.names = data, check.names = F)
...
test_x <- read.table("UCI HAR Dataset/test/X_test.txt", col.names = data, check.names = F)
...
act_labels <- read.table("UCI HAR Dataset/activity_labels.txt", col.names = c('n','text'))

alldata <- cbind(rbind(train_x, test_x), rbind(train_y, test_y), rbind(subject.train, subject.test))
...
write.csv(data4, "clean_dataset.csv", row.names=F)
